...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (blockTime + 5 * 60 * 1000 <= currentTime) {
        return reject("The time elapsed is more than 5 minutes");
      }
      let isValid = bitcoinMessage.verify(message, address, signature);
      if (!isValid) {
        return reject("Your signature is not valid");
      }
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      if (!hash) return reject();
      return resolve(self.chain.filter(block => block.hash === hash)[0]);
    });
  }
